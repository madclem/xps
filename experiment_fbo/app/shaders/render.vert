...
void main() {
    ...
    vec3 pos = a_position * 100.;//texture2D( positions, a_position.xy ).xyz * 1.;
    // vec3 pos = texture2D( positions, a_position.xy ).xyz * 1.;
    pos.z = texture2D( positions, a_position.xy ).x * 20.;
    //pos now contains a 3D position in space, we can use it as a regular vertex

    //regular projection of our position
    gl_Position = u_worldViewProjection * u_world * u_transform * vec4( pos, 1.0 );
